...
exports.updateInvoice = async (req, res) => {
  ...
  // Validate input
  if (!invoiceId || !DueDate || !PaidAmount || !PaymentStatus || !PaymentMethod) {
  ...
  const sql = `
    UPDATE Invoice 
    SET DueDate = ?, PaidAmount = ?, PaymentStatus = ?, PaymentMethod = ? 
    WHERE InvoiceID = ?
  `;

  try {
    // Execute the query
    const [result] = await db.query(sql, [DueDate, PaidAmount, PaymentStatus, PaymentMethod, invoiceId]);
